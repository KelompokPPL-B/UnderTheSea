    ...
    <div class="w-full mx-auto px-4 sm:px-6 lg:px-8 xl:px-12 2xl:px-16 relative z-10">
        ...
        <div class="text-center max-w-3xl mx-auto mb-14">
            ...
            <p class="text-lg text-gray-600 font-medium mb-6">
                Explore the diverse and fascinating fish species in our oceans.
            </p>

            <!-- Search + Sort Header -->
            <div class="flex flex-col md:flex-row justify-center items-center gap-4 w-full mt-6 max-w-4xl mx-auto px-4 z-30 relative">
                <form method="GET" action="{{ route('ikan.index') }}" class="w-full md:flex-1 max-w-2xl relative group search-form animate-fade-in" novalidate>
                    ...
                    <div class="relative bg-white/80 backdrop-blur-md rounded-full p-1.5 flex items-center shadow-xl border border-white/50">
                        ...
                        <input
                            ...
                            placeholder="Cari ikan, habitat, atau deskripsi..."
                ...
                <div class="flex justify-between items-start mb-10">
                    ...
                    @auth
                        @if(auth()->user()->isAdmin())
                            <a href="{{ route('ikan.create') }}" class="bg-gradient-to-r from-ocean-600 to-emerald-500 hover:from-ocean-700 hover:to-emerald-600 text-white font-bold py-2.5 px-5 rounded-xl shadow-md hover:shadow-emerald-500/20 transform hover:-translate-y-0.5 transition-all duration-300 flex items-center gap-2 text-sm">
                        ...
                        <div class="flex flex-wrap items-end gap-8">
                            ...
                            @isset($habitatList)
                                <div class="flex flex-col gap-2">
                                    ...
                                    <select name="habitat" class="select select-bordered w-60">
                            ...
                            @isset($statusList)
                                <div class="flex flex-col gap-2">
                                    ...
                                    <select name="status" class="select select-bordered w-60">
                            ...
                            <div class="ml-10 flex items-end gap-4">
                                ...
                                <a href="{{ route('ikan.index') }}" class="btn btn-ghost h-12 min-h-12 px-6 text-sm font-semibold text-gray-500">
                                    Reset
                                </a>
                            </div>
                        </div>

                        @if(($filterHabitat ?? '') || ($filterStatus ?? '') || $currentSort !== 'newest')
                        ...
                        @foreach($fishList as $item)
                            <div class="group bg-white rounded-[1.5rem] shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] hover:shadow-[0_12px_40px_-8px_rgba(0,0,0,0.15)] border border-gray-100/80 overflow-hidden flex flex-col transition-all duration-300 transform hover:-translate-y-1.5">
                                ...
                                <div class="relative h-60 w-full overflow-hidden bg-gray-100">
                                    @if($item->gambar)
                                        <img src="{{ asset('storage/' . $item->gambar) }}" alt="{{ $item->nama }}" class="w-full h-full object-cover transition-transform duration-700 ease-out group-hover:scale-110" loading="lazy">
                                    @else
                                ...
                                <div class="p-6 flex flex-col flex-grow bg-white relative">
                                    ...
                                    <div class="pt-5 border-t border-gray-100/80 mt-auto">
                                        <div class="flex gap-2.5">
                                            <a href="{{ route('ikan.show', $item->id_ikan) }}" class="flex-1 bg-ocean-100 hover:bg-ocean-600 text-ocean-800 hover:text-white font-bold py-2.5 rounded-xl text-center transition-all duration-300 text-sm border border-ocean-200 hover:border-transparent">
                                                Lihat Detail
                                            </a>

                                            @auth
                                                @if(auth()->user()->isAdmin())
                                                    <a href="{{ route('ikan.edit', $item->id_ikan) }}" class="bg-amber-50 hover:bg-amber-500 text-amber-600 hover:text-white font-bold py-2.5 px-3.5 rounded-xl text-center transition-colors text-sm border border-amber-100 hover:border-amber-500" title="Edit">
