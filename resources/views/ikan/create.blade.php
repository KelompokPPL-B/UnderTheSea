        ...
        <div class="bg-white rounded-lg shadow-lg overflow-hidden">
            <div class="p-6 sm:p-8">
                <div class="mb-8">
                    ...
                    <p class="text-sm sm:text-base text-gray-600">Add a new fish species to the database</p>
                </div>
                ...
                <form method="POST" action="{{ route('ikan.store') }}" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="scientific_name" value="{{ old('scientific_name') }}" class="w-full px-4 py-2 border-2 border-gray-200 rounded-lg" placeholder="Enter scientific name (optional)">
                    ...
                    <div>
                        ...
                        <input type="text" name="habitat" value="{{ old('habitat') }}" required class="w-full px-4 py-2 border-2 {{ $errors->has('habitat') ? 'border-red-500' : 'border-gray-200' }} rounded-lg" placeholder="Enter habitat">
                        @error('habitat')<p class="text-red-600 text-sm mt-1">{{ $message }}</p>@enderror
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-gray-900 mb-2">Description *</label>
                        <textarea name="description" rows="4" required class="w-full px-4 py-2 border-2 {{ $errors->has('description') ? 'border-red-500' : 'border-gray-200' }} rounded-lg" placeholder="Describe the fish">{{ old('description') }}</textarea>
                        @error('description')<p class="text-red-600 text-sm mt-1">{{ $message }}</p>@enderror
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-gray-900 mb-2">Diet</label>
                        <input type="text" name="diet" value="{{ old('diet') }}" class="w-full px-4 py-2 border-2 border-gray-200 rounded-lg" placeholder="Diet (optional)">
                    ...
                    <div>
                        ...
                        <input type="text" name="size" value="{{ old('size') }}" class="w-full px-4 py-2 border-2 border-gray-200 rounded-lg" placeholder="Size (optional)">
                    ...
                    <div>
                        ...
                        <input type="text" name="conservation_status" value="{{ old('conservation_status') }}" class="w-full px-4 py-2 border-2 border-gray-200 rounded-lg" placeholder="e.g. Vulnerable">
                    ...
                    <div class="flex flex-col sm:flex-row gap-3 sm:gap-4 pt-6 border-t border-gray-200">
                        <button type="submit" class="flex-1 px-6 py-3 bg-blue-500 hover:bg-blue-600 text-white rounded-lg font-medium">Create Fish</button>
